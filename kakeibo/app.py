...
def index():
    ...
    yasai_labels = db.execute(label_query, ("野菜", session["user_id"],)).fetchall()[0]
    yasai_prices = db.execute(price_query, ("野菜", session["user_id"],)).fetchall()[0]
    kakou_labels = db.execute(label_query, ("加工食品", session["user_id"],)).fetchall()[0]
    kakou_prices = db.execute(price_query, ("加工食品", session["user_id"],)).fetchall()[0]
    niku_labels = db.execute(label_query, ("食肉・鶏卵", session["user_id"],)).fetchall()[0]
    niku_prices = db.execute(price_query, ("食肉・鶏卵", session["user_id"],)).fetchall()[0]
    gyokai_labels = db.execute(label_query, ("魚介類", session["user_id"],)).fetchall()[0]
    gyokai_prices = db.execute(price_query, ("魚介類", session["user_id"],)).fetchall()[0]
    db.close()
