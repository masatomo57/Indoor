...
def index():
    ...
    yasai_labels = db.execute("SELECT calculated FROM otoku WHERE kind=? AND user_id=?", ("野菜", session["user_id"],)).fetchall()
    yasai_prices = db.execute("SELECT price FROM otoku WHERE kind=? AND user_id=?", ("野菜", session["user_id"],)).fetchall()
    kakou_labels = db.execute("SELECT calculated FROM otoku WHERE kind=? AND user_id=?", ("加工食品", session["user_id"],)).fetchall()
    kakou_prices = db.execute("SELECT price FROM otoku WHERE kind=? AND user_id=?", ("加工食品", session["user_id"],)).fetchall()
    niku_labels = db.execute("SELECT calculated FROM otoku WHERE kind=? AND user_id=?", ("食肉・鶏卵", session["user_id"],)).fetchall()
    niku_prices = db.execute("SELECT price FROM otoku WHERE kind=? AND user_id=?", ("食肉・鶏卵", session["user_id"],)).fetchall()
    gyokai_labels = db.execute("SELECT calculated FROM otoku WHERE kind=? AND user_id=?", ("魚介類", session["user_id"],)).fetchall()
    gyokai_prices = db.execute("SELECT price FROM otoku WHERE kind=? AND user_id=?", ("魚介類", session["user_id"],)).fetchall()
    db.close()
